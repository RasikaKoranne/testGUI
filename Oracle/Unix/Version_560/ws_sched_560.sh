...
# version 4.1.1 01/03/2005 Handle jobs within jobs
# version 5.6.0 18/04/2007 Handle exports
SCHEDULER_VERSION=5006000 # Scheduler version. Do not alter
# ============================================================================
# SETTINGS
...
do
   ...
   # 26 = invalid number of threads. Could mean oracle is down
   #
   WAITED_FOR=2
   $BIN_DIR/ws_job_wait_560.sh $1 $SCHEDULER_NAME >>$LOG_FILE
   RES=$?
   if [ "$RES" -eq "2" ]
   then
   ...
   then
      ...
      $BIN_DIR/ws_sched_status_560.sh $PARAMS
      exit
   elif [ "$RES" -ne "0" ]
   then
      date
      ...
      then
         STOP_MSG="Invalid number of threads. Possibly due to Oracle down"
      else
         STOP_MSG=`echo "ws_job_wait_560.sh $1 returned a status of $RES" | tr " " "^"`
      fi
      echo "ws_job_wait_560.sh $1 $SCHEDULER_NAME returned a status of $RES"
